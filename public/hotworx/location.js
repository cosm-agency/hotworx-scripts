  ...
  data.forEach((item) => {
    if (item.latitude && item.longitude) {
      const marker = new google.maps.Marker({
        ...
        title: item.name,
        icon:
          "https://scripts.cosmagency.com/hotworx/icons/indi.svg",
      });

      marker.addListener("click", () => {
        document.querySelector("#search_field input").value = item.name;
        search(item.name, ["name", "fromMarkerSelect", "zip-code"]);
      });
    ...
    styles: [
      {
        ...
        width: 54,
        url:
          "https://scripts.cosmagency.com/hotworx/icons/group.svg",
      },
    ],
  });
}
...
function geocodeAddress(address, locations) {
  ...
  geocoder.geocode({ address: address }, (results, status) => {
    if (status === "OK") {
      ...
      console.log('geocode result: ', pos)
...
function getAutocompleteQuery(searchAddress) {
  ...
  return new Promise((resolve, reject) => {
    $.ajax({
      ...
      crossDomain: true,
      cache: true,
      url: `https://maps.googleapis.com/maps/api/place/queryautocomplete/json?key=${apiKey}&components=country:us|country:ie|country:au|country:at|country:pr&input=${searchAddress}`,
      method: "GET",
      success: function (data) {
        resolve(data);
      },
...
function search(searchTerm, fieldNames) {
  ...
  console.log('item', checkFromMarker)
  ...
  return new Promise((resolve, reject) => {
    ...
    if (n !== 0) {
      for (let index = 0; index < n; index++) {
        ...
        // console.log('distance: ', locations[index][
        //   "zip-code"
        // ], locations[index][
        //   "distanceToCurrentPos"
        // ])
...
function renderClosestLocations(n, step = "none") {
  const closestLocations = entries
    ...
    .slice(0, n)
    .map((item) => {
      if (item['zip-code'] === document.querySelector('#search_field input').value) {
        item['distanceToCurrentPos'] = 0;
      }
      return item;
    })
    .sort(getComparatorByProp("distanceToCurrentPos", "asc"))
